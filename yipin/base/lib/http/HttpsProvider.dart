...
import 'package:base/http/ReqCallBack.dart';

import 'package:base/Config.dart';
import 'package:base/http/Params.dart';
import 'package:base/http/ValueUtil.dart';
...
//    _dio.interceptors.add(DioInterceptors()); //开启请求日志
  ...
  request(String url, {Map<String,dynamic> data,String method, CancelToken cancelToken,
    ...
    try {
      Response response = await _dio.post(
          url, data: data, cancelToken: cancelToken);
  ...
  Future<dynamic> postFile(String url,Params data,{method,
    ...
    FormData formData = FormData.fromMap(data.map);
...
//    return false;
  ...
  Future<dynamic> postP(String url,Params param,{ReqCallBack callBack,CancelToken cancelToken})  {
    return  _inst.request(url, data: param.map, cancelToken: cancelToken,callBack: callBack,
        baseUrl: getPhpUrl()
    );
  }
...
class DioInterceptors extends InterceptorsWrapper {
  ...
  void onError(DioError err, ErrorInterceptorHandler handler) {
    ...
    print("ERROR[${err?.response?.statusCode}] => PATH: ${err?.response?.requestOptions?.path}");
